...
interface LobstersService {
    @GET("page/{index}.json")
    suspend fun getPage(@Path("index") index: Int): List<Story>

    @GET("s/{short_id}.json")
    suspend fun getStory(@Path("short_id") shortId: ShortId): Story
...
data class Story(
    ...
    @field:Json(name = "submitter_user") val submitter: User,
